...
export async function startMcpServer(config) {
    ...
    async function BatchExecuteMcpHandler(args) {
        const parsedArgs = validateAndParseInput(args, BatchExecuteRequestSchema);
        try {
    ...
    async function AwaitToolDefinitionMcpHandler(args) {
        const parsedArgs = validateAndParseInput(args, AwaitRequestSchema);
        try {
