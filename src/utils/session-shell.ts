...
export class SessionShell {
     ...
     */
    private getShellSpawnInfo(): { command: string; args: string[] } {
     ...
     */
    private formatCommandWithMarker(command: string, marker: string): string {
        switch (this.shellType) {
            case ShellType.CMD:
     ...
     */
    public async executeSequence(commands: string[]): Promise<SessionResult> {
     ...
     */
    public static isShellSupported(shellType: ShellType): boolean {
     ...
     */
    private cleanPwdOutput(rawOutput: string): string {
